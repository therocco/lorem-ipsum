/**
 * <lorem-ipsum>
 * @file Placeholder text generator built with HTML Custom Elements v1
 * @author: Rocco Augusto <rocco@nerdofsteel.com>
**/
class LoremIpsum extends HTMLElement {
    /**
     * Create <lorem-ipsum>.
     * @property {string} type - Type of Lorem Ipsum to create: (paragraph, words, or list)
     * @property {number|number[]} length - Length can be a number or array of number ranges: (e.g. 5, [5, 10]).
    */
    constructor() {
        super();

        this._shadowDOM = this.createShadowRoot();  // create shadow dom
        this._dictionary = [    // set default attribute values
            ...
            "sed", "euismod", "nisi", "porta", "lorem", "mollis", "aliquam", "ut",
    ...
    */
    get length() {
        // grab length attribute
        let length = this.getAttribute('length');
        return length ? JSON.parse(length) : 5;
    }
    ...
    */
    set length(value) {
        this.setAttrbute('length', JSON.stringify(value));
    }
    ...
    */
    get type() {
        ...
        return typeof type !== 'undefined' ? type : 'paragraph';
    }

    /**
     * Set the type value.
     * @return {number|number[]} value length value.
    */
    set type(value) {
        // set type attribute
        this.setAttrbute('type', value);
    }
    ...
    */
    attributeChangedCallback(name, oldValue, newValue) {
        // set attribute if it doesn't already exit
        if(!this[name]) {
            let self = document.querySelector(this);
    ...
    */
     ...
     * @property {number} min - Minimum value to randomly generate.
     * @property {number} max - Maximum value to randomly generate.
    */
     ...
     * @property {string} type - Type of Lorem Ipsum to generate (paragraph, words, list).
     * @property {number} count - Amount of Lorem Ipsum content to generate.
    */
    buildText(type, count) {
        var text = [];
        var random = 0;

        switch(type) {
            case 'paragraph':
                random = this.randomLength(10, 20);
                text.push(this.buildText('sentence', random));
                return text.join('\n');
            case 'sentence':
                for (let i = 0; i < count; i++) {
                    let size = this.randomLength(5, 10);
                    let words = this.buildText('words', size).split(' ');
            ...
            case 'list':
                for (let i = 0; i < count; i++) {
                    let size = this.randomLength(1, 5);
                    let words = this.buildText('words', size).split(' ');
                    words[0] = words[0].substr(0, 1).toUpperCase() + words[0].substr(1);
                    text.push(`<li>${(words.join(' ') + '.').replace(/(\.\,|\,\.)/g, '.')}</li>`);
                }
                return text.join('');
            case 'words':
                let words = [];
                for(let i = 0; i < count; i++) {
                    random = this.randomLength(0, this._dictionary.length-1);
                    words.push(this._dictionary[random]);
                }
    ...
    /**
     * Generate the Shadow DOM template
    **/
    buildShadowDom() {
        let html = [];
        let length = this.length || 5;
        let type = this.type || 'paragraph';
        let [min, max] = typeof length === 'object' ? length : [length, length];
        let size = this.randomLength(min, max);
        let tag = type === 'list' ? 'ul' : (type === 'words' ? 'span' : 'p');

        if(this.type === 'paragraph') {
            for(let i = 0; i < length; i++) {
                html.push(`<${tag}>${this.buildText(this.type, size)}</${tag}>`);
            }
        }
        else {
            html.push(`<${tag}>${this.buildText(this.type, size)}</${tag}>`);
    ...
    **/
    render() {
        ...
        let template = this.getAttribute('type') ? this.buildShadowDom() : '';
